...
{- In lesson 9, we introduced the Attoparsec library and began writing a parser
for HTTP requests. This will allow our web server to understand what its clients
are asking it for. -}
...
{- In lesson 10, we wrote a function to traverse the list of header fields and
figure out what they tell us about the body encoding. -}

messageBodyParser :: [HeaderField] -> Parser (Maybe MessageBody)
messageBodyParser = parserForBodyEncoding . discernBodyEncoding
...
discernBodyEncoding headerFields =
    let
        allResults =
          do
            maybes <- traverse headerFieldBodyEncoding headerFields
    ...
    in
        case allResults of
            ...
            Right (_ : _) -> Left "Cannot determine body encoding; \
                                  \there are too many relevant headers"
...
headerFieldBodyEncoding (HeaderField name value)
    ...
    | nameIs "Transfer-Encoding" =
        do
          transferCodings <- parseFieldValue transferEncodingParser value
...
{- This is the oversimplified Transfer-Encoding parser that we
presented for the sake of discussing why it is incorrect. -}

parseTransferEncodingSimple :: BS.ByteString -> [Text]
parseTransferEncodingSimple =
    map Text.strip .
...
{- The exercise asked you to write a parser for a comma-separated list.
...
unquotedElementParser =
    P.takeWhile (\x -> not (P.inClass " ,\"" x))

quotedElementParser :: Parser BS.ByteString
quotedElementParser =
    do
        _ <- P.string (ASCII.pack "\"")
        x <- P.takeWhile (\x -> not (P.inClass "\"" x))
        _ <- P.string (ASCII.pack "\"")
        return x

------------------------------------------

{- The exercise was a simplified version of the parser for the
Transfer-Encoding header field value. We did not discuss this more
complicated parser in the lesson, but it is given below. -}

data TransferCoding =
    TransferCoding TransferCodingName (Maybe TransferParameter)
    deriving (Eq, Show)

newtype TransferCodingName =
    TransferCodingName BS.ByteString
    deriving (Eq, Show)

data TransferParameter =
    TransferParameter BS.ByteString BS.ByteString
    deriving (Eq, Show)

isChunkedTransferCoding :: TransferCoding -> Bool
isChunkedTransferCoding (TransferCoding name _) =
    getEquivalence transferCodingNameEquivalence name chunkedTransferCodingName

chunkedTransferCodingName :: TransferCodingName
chunkedTransferCodingName = TransferCodingName (ASCII.pack "chunked")

-- All transfer-coding names are case-insensitive
transferCodingNameEquivalence :: Equivalence TransferCodingName
transferCodingNameEquivalence =
    contramap f (defaultEquivalence @String)
  where
    f :: TransferCodingName -> String
    f (TransferCodingName x) = Char.toLower <$> ASCII.unpack x

-- Transfer-Encoding = 1#transfer-coding
transferEncodingParser :: Parser (NonEmpty TransferCoding)
transferEncodingParser =
    oneOrMoreParser transferCodingParser <?> "Transfer-Encoding"
...
transferCodingParser :: Parser TransferCoding
transferCodingParser =
    do
        x <- transferCodingNameParser
        ...
        y <- optional $
            do
                ...
                _ <- P.string (ASCII.pack ";")
                _ <- owsParser
                transferParameterParser

        return (TransferCoding x y)

    <?> "transfer-coding"

transferCodingNameParser :: Parser TransferCodingName
transferCodingNameParser =
    do
        x <- tokenParser
        return (TransferCodingName x)

-- transfer-parameter = token "=" ( token / quoted-string )
transferParameterParser :: Parser TransferParameter
transferParameterParser =
    do
        x <- tokenParser
        _ <- P.string (ASCII.pack "=")
        y <- tokenParser <|> quotedStringParser
        return (TransferParameter x y)
...
quotedStringParser =
    do
        _ <- P.string (ASCII.pack "\"")
        xs <- many (qdtextParser <|> quotedPairParser)
        _ <- P.string (ASCII.pack "\"")
        return (ASCII.pack xs)
...
quotedPairParser =
    do
        _ <- P.string (ASCII.pack "\"")
        P.satisfy (\x -> P.inClass " \t" x || isVchar x)

    <?> "quoted-pair"

-- qdtext = HTAB / SP / (VCHAR other than DQUOTE and backslash)
qdtextParser :: Parser Char
qdtextParser =
    P.satisfy
        (\x ->
            P.inClass " \t" x
            || (isVchar x && not (P.inClass "\"\\" x))
        )
...
parserForBodyEncoding x =
    case x of
        ...
        Right (Just (ContentLength n)) ->
            do
                body <- messageBodyParser_contentLength n
                return (Just body)

        Right (Just Chunked) ->
            do
                body <- messageBodyParser_chunked
                return (Just body)
...
messageBodyParser_contentLength :: Natural -> Parser MessageBody
messageBodyParser_contentLength size =
    do
        x <- P.take (fromIntegral @Natural @Int size)
        return (MessageBody (LBS.fromStrict x))

------------------------------------------

messageBodyParser_chunked :: Parser MessageBody
messageBodyParser_chunked =
    do
        chunks <- P.manyTill chunkParser lastChunkParser
        _ <- crlfParser

        return (MessageBody (LBS.fromChunks chunks))

chunkParser :: Parser BS.ByteString
chunkParser =
    do
        size    <- chunkSizeParser
        _       <- crlfParser
        content <- chunkDataParser size
        _       <- crlfParser
...
chunkSizeParser :: Parser Natural
chunkSizeParser =
    hexadecimalParser <?> "chunk-size"

hexadecimalParser :: Parser Natural
hexadecimalParser =
    P.hexadecimal <?> "1*HEXDIG"

chunkDataParser :: Natural -> Parser BS.ByteString
chunkDataParser size =
    P.take (fromIntegral @Natural @Int size) <?> "chunk-data"

lastChunkParser :: Parser ()
lastChunkParser =
    do
        size <- chunkSizeParser
        _ <- crlfParser
    ...
    server $ \socket ->
      do
        ...
        case (LP.parse requestParser lbs) of
            ...
            LP.Fail _remainingInput ctxs msg -> do
                putStrLn "Error"
...
showParseError :: [String] -> String -> String
showParseError ctxs msg =
    case ctxs of
        [] -> msg
        _  -> List.intercalate " > " ctxs ++ ": " ++ msg

------------------------------------------

plainTextUtf8Header :: HeaderField
plainTextUtf8Header =
    contentTypeHeader "text/plain; charset=utf-8"

utf8MessageBody :: String -> MessageBody
utf8MessageBody x =
    MessageBody (UTF8.fromString x)

plainTextUtf8Response :: String -> Response
plainTextUtf8Response x =
    chunkResponse $
        Response
            (StatusLine http_1_1 status200 reasonOK)
            [ plainTextUtf8Header ]
            (Just (utf8MessageBody x))

------------------------------------------

echo :: Request -> Response
echo request =
    plainTextUtf8Response (ppShow @Request request ++ "\n")
